...
Proof.
  ...
  - split. 
    + reflexivity.
    + simpl in H. apply H.
  - split. discriminate H. discriminate H.
Qed.
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P H1. unfold not in H1. intros Q H2.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H. unfold not. intros H1 H2.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intro P. unfold not. intros [H1 H2].
...
Theorem dupla_neg : forall (b : bool),
  negb (negb b) = b.
Proof.
  intros. induction b.
  - simpl. reflexivity.
...
Theorem prop_neg : forall (b : Prop),
  (~~b) = b.
Proof.
  intros. unfold not. 
Abort.

Theorem excluido_3Bool : forall (b:bool),
  orb b (negb b) = true.
Proof.
  intros. induction b.
  - simpl. reflexivity.
...
Theorem prop_excluido_3Bool : forall (P:Prop),
  (P) \/ (~P) -> True.
Proof.
  intros. reflexivity.
Qed.
...
Proof.
  ...
  - intros. apply H in H2. apply H0 in H.
    + apply H.
...
Proof.
  intros t p H. unfold not. intros. destruct H0 as [h H1].
  apply H1 in H.
  destruct H.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. intros [].
    + exists x. split.
      * apply H.
      * left. auto.
    + apply IHl in H. destruct H. exists x0. split.
      * destruct H. apply H. 
